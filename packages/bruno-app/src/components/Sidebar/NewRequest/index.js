...
import { newHttpRequest, newGrpcRequest, newWsRequest, newSioRequest } from 'providers/ReduxStore/slices/collections/actions';
...
const NewRequest = ({ collectionUid, item, isEphemeral, onClose }) => {
  ...
  const getRequestType = (collectionPresets) => {
    ...
    if (collectionPresets.requestType === 'socketio') {
      return 'socketio-request';
    }
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      const isSioRequest = values.requestType === 'socketio-request';
      ...
      if (isGrpcRequest) {
        ...
        dispatch(newWsRequest({
          ...
          .catch((err) => toast.error(err ? err.message : 'An error occurred while adding the request'));
      } else if (isSioRequest) {
        dispatch(newSioRequest({
          requestName: values.requestName,
          filename: filename,
          requestType: values.requestType,
          requestUrl: values.requestUrl,
