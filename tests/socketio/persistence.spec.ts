...
test.describe.serial('socketio persistence', () => {
  let originalUrl = '';
  let originalData = '';
  ...
  test.beforeAll(async () => {
    ...
    if (!originalUrlMatch) {
      throw new Error('url not found in bru file for socketio');
    }
  ...
  test('save new socketio url', async ({ pageWithUserData: page }) => {
    const replacementUrl = 'http://localhost:8083';
